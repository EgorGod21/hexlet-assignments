...
public class UsersServlet extends HttpServlet {
    ...
    private void createUser(HttpServletRequest request,
        ...
        Map<String, String> user =Map.of("id", getNextId(),"firstName",firstName,"lastName",lastName,"email",email);
        if (user.get("firstName").equals("") || user.get("lastName").equals("")){
            RequestDispatcher requestDispatcher = request.getRequestDispatcher("/new.jsp");
            request.setAttribute("user", user);
            request.setAttribute("error", "Имя или Фамилия не могут быть пустыми");
    ...
    private void updateUser(HttpServletRequest request,
        ...
        Map<String, String> user = getUserById(id);
        String emailStart=user.get("email");
        ...
        if (user.get("firstName").equals("") || user.get("lastName").equals("") || user.get("email").equals("")){
            RequestDispatcher requestDispatcher = request.getRequestDispatcher("/edit.jsp");
            request.setAttribute("user", user);
            request.setAttribute("error", "Данные введены не корректно");
            response.setStatus(422);
            requestDispatcher.forward(request, response);
        }
        else{
